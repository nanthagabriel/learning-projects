...
# The maximum number of attempts given.
num_of_attempts = 1
...
# Loop till '10' attempt are over. 
while guess != secret_number and num_of_attempts < max_num_of_attempts:
    
    # Clues are given to narrow down the user's guesses. 
    if guess < secret_number:
        print("uh uh aah .. that's too low")
    else :
        print("So close! .. but thats too high")

    # Plus '1' every time the user fails an attempt.
    num_of_attempts += 1

    # Countdown of the number of attempts the user has and user input for another attempt.
    if num_of_attempts < max_num_of_attempts:
         guess = int(input(f"\nDon't give up! .. You have {num_of_attempts} / {max_num_of_attempts} left .. C'mon now make another guess! : "))
